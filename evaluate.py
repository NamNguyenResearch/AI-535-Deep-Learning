...
def plot_rdp(
    ...
    plt.ylabel("Classification Accuracy Loss")
    ...
    assert all(
        item not in plot_settings_dict.items() for item in ("s", "c", "edgecolors")
    )
    ...
    if line_segments == "refined":
        ...
        for idx in range(0, len(xl), 2):
            if line_colors is not None:
                ...
                color = "silver"
            plt.plot(
                xl[idx : idx + 2], yl[idx : idx + 2], color=color, lw=1.2, zorder=0
            )
    ...
    name = (
        f"RDP_Tradeoff_{rdp_name_suffix}.pdf" if rdp_name_suffix else "RDP_Tradeoff.pdf"
    )
...
def plot_training_losses_(experiment_dir, d_avg, p_avg):
    with (
        open(f"{experiment_dir}/_losses.csv", "r") as f,
        ...
        plot_training_losses(
            range(len(reader[1:])),
            distortion_losses_training,
            perception_losses_training,
            experiment_dir,
            settings,
            d_avg,
            p_avg,
...
def plot_training_losses(
    epoches, distortion, perception, experiment_dir, settings, d_avg, p_avg
):
    """
    ...
    if settings["mode"] == "base":
        ...
        fig3.suptitle(
            "Distortion = %.3f, Rate = %.3f, $d_W$ = %.3f" % (d_avg, rate, p_avg)
        )
        ...
        fig3.suptitle(
            "Distortion = %.3f, Rate 1 = %.3f, Rate 2 = %.3f, $d_W$ = %.3f"
            % (d_avg, rate_1, rate_2, p_avg)
        )
        fig3.savefig(f"{experiment_dir}/losses.png")
    else:  # mode == reduced
        rate_1, rate_0 = -1, -1
        ...
        fig3.suptitle(
            "Distortion = %.3f, Rate 1 = %.3f, Rate 0 = %.3f, $d_W$ = %.3f"
            % (d_avg, rate_1, rate_0, p_avg)
        )
        fig3.savefig(f"{experiment_dir}/losses.png")
...
def eval_mode_two(eval_args):
    ...
    print("Experiment Directory: ", experiment_dirs, "\n")
    ...
    for experiment_dir in experiment_dirs:
        with open(f"{experiment_dir}/_settings.json") as f:
            ...
            if mode == "base":
                ...
                if train_args.L_1 > 0 and train_args.L_2 > 0:
                    n_bits = train_args.latent_dim_1 * math.log2(
                        train_args.L_1
                ...
                if train_args.submode == "joint_reduced_reduced_1":
                    ...
                    Lambda_text = (
                        f"$\lambda_1$={train_args.Lambda_base} \n "
        ...
        with open(f"{experiment_dir}/_accuracy.csv") as f:
            losses = f.readlines()
            ...
            for epoch, distortion_loss, perception_loss in losses[-n_losses:]:
                try:
                    ...
                    print(
                        "Most like scenario is either an experiment was overwritten then cancelled, deleting all _losses.csv data"
                    )
                    print(
                        f"Or the number of losses used to evaluate average loss ({n_losses}) is less than the number of recorded losses ({len(losses)})"
                    )
                    raise ValueError(ve)
            ...
            if mode == "base":
                ...
                multistage_points[
                    (
                        train_args.latent_dim_2,
                        train_args.L_2,
                        train_args.Lambda_refined,
                        train_args.latent_dim_1,
                        train_args.L_1,
                        train_args.Lambda_base,
                    )
                ] = {
                    "location": (distortion_loss_avg, perception_loss_avg),
                    "base_point_setting": (
        ...
        with open(f"{experiment_dir}/_evaluation2.csv", "w") as f:
            if train_args.mode == "base":
                ...
                f.write(
                    "n_bits,distortion_loss,perception_loss,latent_dim_1,latent_dim_2,L_1,L_2,Lambda_base,Lambda_refined\n"
                )
                f.write(f"""
                ...
                f.write(
                    "n_bits,distortion_loss,perception_loss,latent_dim_1,latent_dim_0,L_1,L_0,Lambda_base,Lambda_reduced\n"
                )
                f.write(f"""
                    {n_bits},{distortion_loss_avg},{perception_loss_avg},{train_args.latent_dim_1},
    ...
    for setting, details in multistage_points.items():
        ...
        line_settings.append(
            setting
        )  # Records latent_dim_2, L_2, Lambda_2, latent_dim_1, L_1, Lambda_1,
...
def preview_reduced_decoders(eval_args):
    ...
    reduced_paths = get_secondary_model_dirs(
        [
            base_path,
        ],
        filtering="reduced_only",
    )
    ...
    _, test_dataloader, unnormalizer = load_dataset(
        dataset_name,
        eval_args.test_batch_size,
        eval_args.test_batch_size,
        shuffle_test=eval_args.shuffle_test,
    )

    encoder1 = Encoder1(base_model_args).to(device)
    encoder1.load_state_dict(
        torch.load(os.path.join(base_path, "encoder1.ckpt"), map_location=device)
    )
    encoder1.eval()
    ...
    for reduced_path in reduced_paths:
        ...
        reduced_dims = str_values_to_tensor(
            reduced_model_args.reduced_dims, delimiter=","
        ).to(device)

        decoder0 = Decoder0(reduced_model_args).to(device)
        decoder0.load_state_dict(
            torch.load(os.path.join(reduced_path, "decoder0.ckpt"), map_location=device)
        )
        decoder0.eval()
    ...
    if not os.path.isfile(os.path.join(base_path, "_reduced", "change_decoder.png")):
        save_image(
            unnormalizer(all_images),
            os.path.join(base_path, "_reduced", "change_decoder.png"),
            nrow=eval_args.test_batch_size,
            normalize=True,
        )
    else:
        i = 0
        while os.path.isfile(
            os.path.join(base_path, "_reduced", f"change_decoder_{i}.png")
        ):
            i += 1
        save_image(
            unnormalizer(all_images),
...
def compare_e2e_vs_universal(eval_args):
    ...
    main_base_subpath = os.path.join(
        f"{eval_args.latent_dim_1}-{eval_args.L_1}",
        "Lambda={:.5f}".format(eval_args.Lambda_base),
    )
    ...
    reduced_paths = get_secondary_model_dirs(
        ...
        filtering="reduced_only_same_dim",
    )
    ...
    encoder1 = Encoder1(base_model_args).to(device)
    encoder1.load_state_dict(
        torch.load(os.path.join(main_base_path, "encoder1.ckpt"), map_location=device)
    )
    encoder1.eval()
    ...
    save_image(
        unnormalizer(x_test.data[: n * n]),
        os.path.join(savepath, "_originals.png"),
        nrow=n,
    )
    for reduced_path in reduced_paths:
        ...
        with torch.no_grad():
            # Important: Permute over z entries as done in selection phase
            # for each element of the batch
            u1_test = uniform_noise(
                [x_test.size(0), eval_args.latent_dim_1], encoder1.alpha
            ).to(device)
            z = encoder1(x_test, u1_test).index_select(1, reduced_dims)
            x_test_recon = decoder0(
                z, u1_test.index_select(1, reduced_dims)
            )  # permute noise

            # Save square grid of images
            save_image(
                unnormalizer(x_test_recon.data[: n * n]),
                os.path.join(
                    savepath,
                    "Lambda={:.5f}-reduced.png".format(
                        reduced_model_args.Lambda_reduced
                    ),
                ),
                nrow=n,
            )
    ...
    other_base_Lambda_list = str_values_to_list(
        eval_args.other_base_Lambda, delimiter=",", dtype=float
    )
    other_base_paths = [
        os.path.join(
            eval_args.experiment_path_pre,
    ...
    for other_base_path in other_base_paths:
        ...
        encoder1.load_state_dict(
            torch.load(
                os.path.join(other_base_path, "encoder1.ckpt"), map_location=device
            )
        )
        encoder1.eval()
        ...
        decoder1.load_state_dict(
            torch.load(
                os.path.join(other_base_path, "decoder1.ckpt"), map_location=device
            )
        )
        decoder1.eval()
        ...
        with torch.no_grad():
            ...
            save_image(
                ...
                os.path.join(
                    savepath,
                    "Lambda={:.5f}-e2e.png".format(other_base_model_args.Lambda_base),
                ),
                nrow=n,
            )
...
def compare_e2e_vs_refined(eval_args):
    ...
    savepath = os.path.join(
        eval_args.experiment_path_pre,
        f"{latent_dim_1_lower}-{L_1_lower}",
        "_compare_refined",
    )
    os.makedirs(savepath, exist_ok=True)
    print(f"savepath: {savepath}")
    ...
    main_base_path = os.path.join(eval_args.experiment_path_pre, main_base_subpath)
    print(f"Base path: {main_base_path}")
    refined_paths = get_secondary_model_dirs(
        [
            main_base_path,
        ],
        filtering="refined_only",
    )
    ...
    for refined_path in refined_paths:
        ...
        encoder2.load_state_dict(
            torch.load(os.path.join(refined_path, "encoder2.ckpt"), map_location=device)
        )
        encoder2.eval()

        decoder2 = Decoder2(refined_model_args).to(device)
        decoder2.load_state_dict(
            torch.load(os.path.join(refined_path, "decoder2.ckpt"), map_location=device)
        )
        decoder2.eval()
        ...
        with torch.no_grad():
            ...
            # for each element of the batch
            u1_test = uniform_noise(
                [x_test.size(0), latent_dim_1_lower], encoder1.alpha
            ).to(device)
            u2_test = uniform_noise(
                [x_test.size(0), refined_model_args.latent_dim_2], encoder2.alpha
            ).to(device)
            ...
            save_image(
                ...
                os.path.join(
                    savepath,
                    f"refined-{refined_model_args.latent_dim_2}-{refined_model_args.L_2}-"
                    + "Lambda={:.5f}.png".format(refined_model_args.Lambda_refined),
                ),
                nrow=n,
            )
    ...
    for latent_dim_1, L_1 in zip(
        [latent_dim_1_lower, latent_dim_1_higher], [L_1_lower, L_1_higher]
    ):
        other_base_paths += [
            os.path.join(
                eval_args.experiment_path_pre,
    ...
    for other_base_path in other_base_paths:
        ...
        with torch.no_grad():
            u1_test = uniform_noise(
                [x_test.size(0), other_base_model_args.latent_dim_1], encoder1.alpha
            ).to(device)
            z = encoder1(x_test, u1_test)
            x_test_recon = decoder1(z, u1_test)
            ...
            save_image(
                ...
                os.path.join(
                    savepath,
                    f"e2e-{other_base_model_args.latent_dim_1}-{other_base_model_args.L_1}-"
                    + "Lambda={:.5f}.png".format(other_base_model_args.Lambda_base),
                ),
                nrow=n,
            )
...
def testfunc(eval_args):
    ...
    main_base_subpath = os.path.join(
        f"{latent_dim_1}-{L_1}", "Lambda={:.5f}".format(eval_args.Lambda_base)
    )
    ...
    decoder1.load_state_dict(
        torch.load(os.path.join(main_base_path, "decoder1.ckpt"), map_location=device)
    )
    decoder1.eval()
...
# python evaluate.py -experiment_path_pre experiments/M1 -mode 2 --rdp_name_suffix "example_plot" --test_batch_size 5000 --n_losses 3 --colorbar "bits" --text "np_argsort_excluding" --line_segments "none" --climits 0 10 --Lambda_base 0.015 --latent_dim_1 3 --L_1 3 --shuffle_test 1 --random_seed 1

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "-experiment_path_pre", type=str, help="folder (including experiments/)"
    )
    parser.add_argument(
        "-mode", type=str, default="2", help="folder under experiments/"
    )
    # for eval mode two
    parser.add_argument(
        "--rdp_name_suffix", type=str, default="", help="suffix for plot title"
    )
    parser.add_argument(
        "--n_eval",
        type=int,
        default=3,
        help="[CALCULATE LOSSES MODE] number of batch estimates over entire test set to use for evaluating perception loss",
    )
    parser.add_argument(
        "--test_batch_size",
        type=int,
        default=5000,
        help="test set batch size (default: 1000)",
    )
    parser.add_argument(
        "--n_losses",
        type=int,
        default=3,
        help="[SAVED LOSSES MODE] number of losses to average over",
    )
    parser.add_argument(
        "--colorbar",
        type=str,
        default="bits",
        help="Colorbar to display - bits or dims",
    )
    parser.add_argument(
        "--text", type=str, default="Lambdas", help="Text to display - bits or dims"
    )
    parser.add_argument(
        "--line_segments",
        type=str,
        default="none",
        help="Line segments on plot between base/multistage models",
    )
    parser.add_argument(
        "--climits",
        nargs=2,
    ...
    parser.add_argument(
        "--Lambda_base", type=float, default=0.0, help="Lambda value for base model"
    )
    parser.add_argument(
        "--latent_dim_1",
        type=int,
        default=4,
        help="Latent dimension size of base model",
    )
    parser.add_argument(
        "--L_1",
        type=int,
        default=4,
        help="Number of quantization levels for base model",
    )
    parser.add_argument(
        "--shuffle_test",
        type=int,
        default=1,
        help="Whether to use random test samples or not",
    )
    parser.add_argument("--random_seed", type=int, default=1, help="Random seed")
    parser.add_argument(
        "--custom_selection", type=str, default=None, help="custom selection"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Other base Lambdas for compare_1stage_vs_2stage",
    )
    parser.add_argument(
        "--num_pics", type=int, default=5, help="n, for n x n picture grids"
    )
    parser.add_argument(
        "--latent_dim_1s",
        nargs=2,
        type=int,
        default=[3, 6],
        help="Latent dimension size of base model at higher and lower rate",
    )
    parser.add_argument(
        "--L_1s",
        nargs=2,
        type=int,
        default=[3, 3],
        help="Number of quantization levels for base model at higher and lower rate",
    )
    ...
    if eval_args.mode == "2":
        ...
        line_colors = [
            "silver" if line_setting[5] == 0 else "grey"
        ...
        climits = [
            float(eval_args.climits[0]) if isfloat(eval_args.climits[0]) else None,
        ...
        plot_rdp(
            ...
            eval_args.experiment_path_pre,
            texts=texts,
            colorbar=coloarbar_label,
            plot_settings=plot_settings,
            line_segments=eval_args.line_segments,
            lines=lines,
            line_colors=line_colors,
            climits=climits,
            rdp_name_suffix=eval_args.rdp_name_suffix,
        )
