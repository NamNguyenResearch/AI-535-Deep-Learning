...
from models import *
from utils import *
from quantizer_entropy import (
    base_estimate_entropy_,
    refined_estimate_entropy_,
    reduced_estimate_entropy_,
)
...
def is_progress_interval(args, epoch):
    return epoch == args.n_epochs - 1 or (
        args.progress_intervals > 0 and epoch % args.progress_intervals == 0
    )
...
def compute_gradient_penalty(D, real_samples, fake_samples):
    ...
    alpha = torch.FloatTensor(np.random.random((real_samples.size(0), 1, 1, 1))).to(
        device
    )
    ...
    fake = Variable(
        torch.FloatTensor(real_samples.shape[0], 1).fill_(1.0), requires_grad=False
    ).to(device)
...
def train_base(args, device):
    # =============================================================================
    # Load classifier
    # Define the classifier model before loading it
    model_classifier = Net().to(device)

    trained_model_path = "./trained_classifier.pth"
    if os.path.exists(trained_model_path):
        model_classifier.load_state_dict(torch.load(trained_model_path))
        model_classifier.eval()
        print(f"Loaded trained classifier from {trained_model_path}")
    else:
        print(
            "[Classifier] No pre-trained classifier found. Make sure to provide one if CE is required."
        )
    # =============================================================================
    ...
    assert not (args.L_1 > 0 and not args.quantize), (
        f"Quantization disabled, yet args.L_1={args.L_1}"
    )
    ...
    if args.initialize_mse_model:
        ...
        if args.Lambda_base > 0:
            ...
            # Check args match
            with open(
                os.path.join(args.load_mse_model_path, "_settings.json"), "r"
            ) as f:
                mse_model_args = json.load(f)
                assert_args_match(
                    mse_model_args,
                    vars(args),
                    ("L_1", "latent_dim_1", "limits", "enc_layer_scale"),
                )
        ...
        if isinstance(args.load_mse_model_path, str):
            ...
            encoder1.load_state_dict(
                torch.load(os.path.join(args.load_mse_model_path, "encoder1.ckpt"))
            )
            decoder1.load_state_dict(
                torch.load(os.path.join(args.load_mse_model_path, "decoder1.ckpt"))
            )
            discriminator1.load_state_dict(
                torch.load(
                    os.path.join(args.load_mse_model_path, "discriminator1.ckpt")
                )
            )

    # Configure data loader
    train_dataloader, test_dataloader, unnormalizer = load_dataset(args.dataset, args.batch_size, args.test_batch_size, shuffle_train=True)
    test_set_size = len(test_dataloader.dataset)

    # Optimizers
    optimizer_E1 = torch.optim.Adam(
        encoder1.parameters(),
        lr=args.lr_encoder,
        betas=(args.beta1_encoder, args.beta2_encoder),
    )
    optimizer_G1 = torch.optim.Adam(
        decoder1.parameters(),
        lr=args.lr_decoder,
        betas=(args.beta1_decoder, args.beta2_decoder),
    )
    optimizer_D1 = torch.optim.Adam(
        discriminator1.parameters(),
        lr=args.lr_critic,
        betas=(args.beta1_critic, args.beta2_critic),
    )
    ...
    cross_entropy_loss = torch.Tensor([-1])  # Initialize cross_entropy_loss
    # =============================================================================
    ...
    for epoch in range(args.n_epochs):
        ...
        for i, (x, y) in enumerate(train_dataloader):
            ...
            if i % n_cycles != 1:
                ...
                gradient_penalty = compute_gradient_penalty(
                    discriminator1, x.data, x_recon.data
                )
                # Adversarial loss
                disc_loss = (
                ...
                classifier_output = model_classifier(x_recon)
                cross_entropy_loss = cross_entropy_criterion(classifier_output, y)

                loss = (args.Lambda_distortion * distortion_loss + Lambda * cross_entropy_loss)
                # =============================================================================
            ...
            if batches_done % 100 == 0:
                with torch.no_grad():  # use most recent results
                    ...
                    perception_loss = -torch.mean(fake_validity) + torch.mean(real_validity)
                print(
                    "[Epoch %d/%d] [Batch %d/%d (batches_done: %d)] [Disc loss: %f] [Perception loss: %f] [Distortion loss: %f] [Cross entropy loss: %f]"
                    % (
                        epoch,
                        args.n_epochs,
                        i,
        ...
        with torch.no_grad():
            ...
            is_entropy_interval = args.entropy_intervals > 0 and (
                epoch < 5 or epoch % args.entropy_intervals == 0
            )
            if is_entropy_interval or (
                (epoch == args.n_epochs - 1 or epoch == 0)
                and args.entropy_intervals != -2
            ):
            ...
            # =============================================================================
            distortion_loss_avg, perception_loss_avg, cross_entropy_loss_avg, accuracy_avg, total_accuracy = 0, 0, 0, 0, 0
            # =============================================================================
            ...
            for j, (x_test, y_test) in enumerate(test_dataloader):
                ...
                # =============================================================================
                distortion_loss, perception_loss, cross_entropy_loss = evaluate_losses(x_test, x_test_recon, y_test, discriminator1, model_classifier)
                
                # Metrics
                distortion_loss_avg += x_test.size(0) * distortion_loss
                perception_loss_avg += x_test.size(0) * perception_loss
                cross_entropy_loss_avg += x_test.size(0) * cross_entropy_loss

                # Accuracy metric
                label_outputs = model_classifier(x_test_recon)

                prediction = torch.max(label_outputs, 1)[1]
                total_accuracy += (prediction == y_test).sum().item()

                # =============================================================================

                if j == 0 and is_progress_interval(args, epoch):
                    save_image(
                        unnormalizer(x_test_recon.data[:120]),
                        f"{experiment_path}/{epoch}_recon.png",
                        nrow=10,
                        normalize=True,
                    )
                    if not saved_original_test_image:
                        save_image(
                            unnormalizer(x_test.data[:120]),
                            f"{experiment_path}/{epoch}_real.png",
                            nrow=10,
                            normalize=True,
                        )
            ...
            cross_entropy_loss_avg /= test_set_size
...
def train_refined(args, device):
    ...
    assert (
        args.L_1 > 0
        and args.latent_dim_1 > 0
        and args.L_2 > 0
        and args.latent_dim_2 > 0
    )
    ...
    encoder1.load_state_dict(
        torch.load(os.path.join(args.load_base_model_path, "encoder1.ckpt"))
    )
    if args.initialize_base_discriminator:
        # Load base discriminator into discriminator2
        assert os.path.isfile(
            os.path.join(args.load_base_model_path, "discriminator1.ckpt")
        ), (
            f"No file {os.path.join(args.load_base_model_path, 'discriminator1.ckpt')} found!"
        )
        discriminator2.load_state_dict(
            torch.load(os.path.join(args.load_base_model_path, "discriminator1.ckpt"))
        )
        print(
            "Successfully loaded base discriminator (parameters not frozen - will continue training)."
        )

    frozen_params(encoder1)  # Only train refining models

    # Configure data loader
    train_dataloader, test_dataloader, unnormalizer = load_dataset(
        args.dataset, args.batch_size, args.test_batch_size, shuffle_train=True
    )
    test_set_size = len(test_dataloader.dataset)
    ...
    for epoch in range(args.n_epochs):
        ...
        for i, (x, _) in enumerate(train_dataloader):
            ...
            if i % n_cycles != 1:
                ...
                # Gradient penalty
                gradient_penalty = compute_gradient_penalty(
                    discriminator2, x.data, x_recon.data
                )
                # Adversarial loss
                disc_loss = (
                    -torch.mean(real_validity)
                ...
                perception_loss = -torch.mean(
                    fake_validity
                )  # + torch.mean(real_validity)
                distortion_loss = criterion(x, x_recon)

                loss = (
                    args.Lambda_distortion * distortion_loss + Lambda * perception_loss
                )
                loss.backward()
            ...
            if batches_done % 100 == 0:
                with torch.no_grad():  # use most recent results
                    real_validity = discriminator2(x)
                    perception_loss = -torch.mean(fake_validity) + torch.mean(
                        real_validity
                    )
                print(
        ...
        with torch.no_grad():
            ...
            for j, (x_test, _) in enumerate(test_dataloader):
                x_test = x_test.to(device)
                u1_test = uniform_noise([x_test.size(0), args.latent_dim_1], alpha1).to(
                    device
                )
                u2_test = uniform_noise([x_test.size(0), args.latent_dim_2], alpha2).to(
                    device
                )
                u_test = torch.cat([u1_test, u2_test], dim=1)
                z = torch.cat(
                    (encoder1(x_test, u1_test), encoder2(x_test, u2_test)), dim=1
                )
                x_test_recon = decoder2(z, u_test)
                distortion_loss, perception_loss = evaluate_losses(
                    x_test, x_test_recon, discriminator2
                )
                distortion_loss_avg += x_test.size(0) * distortion_loss
                perception_loss_avg += x_test.size(0) * perception_loss
...
def train_reduced(args, device):
    ...
    if args.initialize_base_discriminator:
        ...
        discriminator0.load_state_dict(
            torch.load(os.path.join(args.load_base_model_path, "discriminator1.ckpt"))
        )
        print(
            "Successfully loaded base discriminator (parameters not frozen - will continue training)."
    ...
    # Optimizers
    optimizer_G0 = torch.optim.Adam(
        decoder0.parameters(),
        lr=args.lr_decoder,
        betas=(args.beta1_decoder, args.beta2_decoder),
    )
    optimizer_D0 = torch.optim.Adam(
        discriminator0.parameters(),
        lr=args.lr_critic,
        betas=(args.beta1_critic, args.beta2_critic),
    )
    ...
    with open(f"{experiment_path}/_accuracy.csv", "w") as f:
        f.write("epoch,distortion_loss,accuracy_loss\n")
    # =============================================================================
    ...
    for epoch in range(args.n_epochs):
        ...
        for i, (x, y) in enumerate(train_dataloader):
            ...
            if i % n_cycles != 1:
                ...
                u0 = u1.index_select(
                    1, reduced_dims
                )  # Shared randomness over selected dims
                z1 = encoder1(x, u1)
                ...
                # Gradient penalty
                gradient_penalty = compute_gradient_penalty(
                    discriminator0, x.data, x_recon.data
                )
                # Adversarial loss
                disc_loss = (
                    -torch.mean(real_validity)
        ...
        with torch.no_grad():
            ...
            for j, (x_test, y_test) in enumerate(test_dataloader):
                ...
                # =============================================================================
                distortion_loss, perception_loss, cross_entropy_loss = evaluate_losses(x_test, x_test_recon, y_test, discriminator0, model_classifier)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_epochs", type=int, default=25, help="number of epochs of training"
    )
    parser.add_argument(
        "--n_channel", type=int, default=1, help="number of image channels"
    )
    parser.add_argument(
        "--n_critic",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Whether to quantize or not. [Does nothing!] Always active.",
    )
    parser.add_argument(
        "--stochastic",
        type=int,
        default=1,
        help="add noise below quantization threshold (default: True)",
    )
    parser.add_argument(
        "--latent_dim_1", type=int, default=8, help="dimensionality of the latent space"
    )
    parser.add_argument(
        "--latent_dim_2",
        type=int,
        default=-1,
        help="dimensionality of the latent space for refinement model",
    )
    parser.add_argument(
        "--latent_dim_0",
        type=int,
        default=-1,
        help="dimensionality of the latent space for reduced model",
    )
    parser.add_argument(
        "--latent_dim_M1",
        type=int,
        default=-1,
        help="dimensionality of the latent space for joint_reduced_reduced model",
    )
    parser.add_argument(
        "--L_1",
        type=int,
        default=3,
        help="number of quantization levels for base model (default: -1)",
    )
    parser.add_argument(
        "--L_2",
        type=int,
        default=-1,
        help="number of quantization levels for refined model (default: -1)",
    )
    parser.add_argument(
        "--L_0",
        type=int,
        default=-1,
        help="number of quantization levels for reduced model (default: -1)",
    )
    parser.add_argument(
        "--L_M1",
        type=int,
        default=-1,
        help="number of quantization levels for joint_reduced_reduced model (default: -1)",
    )
    parser.add_argument(
        "--limits",
        nargs=2,
        type=float,
        default=[-1, 1],
        help="quanitzation limits (default: (-1,1))",
    )
    parser.add_argument(
        "--Lambda_base",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=-1,
        help="coefficient for perception loss for training refined model (default: -1)",
    )
    parser.add_argument(
        "--Lambda_reduced",
        type=float,
        default=-1,
        help="coefficient for perception loss for training reduced model (default: -1)",
    )
    parser.add_argument(
        "--Lambda_reduced_reduced",
        type=float,
        default=-1,
        help="coefficient for perception loss for training joint_reduced_reduced model (default: -1)",
    )
    parser.add_argument(
        "--Lambda_distortion",
        type=float,
        default=1.0,
        help="coefficient for distortion loss (default: 1.0)",
    )
    parser.add_argument(
        "--Lambda_joint",
        type=float,
    ...
    parser.add_argument(
        "--Lambda_gp", type=float, default=10.0, help="coefficient for gradient penalty"
    )
    parser.add_argument(
        "--lr_encoder", type=float, default=1e-2, help="encoder learning rate"
    )
    parser.add_argument(
        "--lr_decoder", type=float, default=1e-2, help="decoder learning rate"
    )
    parser.add_argument(
        "--lr_critic", type=float, default=2e-4, help="critic learning rate"
    )
    parser.add_argument(
        "--beta1_encoder", type=float, default=0.5, help="encoder beta 1"
    )
    parser.add_argument(
        "--beta1_decoder", type=float, default=0.5, help="decoder beta 1"
    )
    parser.add_argument("--beta1_critic", type=float, default=0.5, help="critic beta 1")
    parser.add_argument(
        "--beta2_encoder", type=float, default=0.9, help="encoder beta 2"
    )
    parser.add_argument(
        "--beta2_decoder", type=float, default=0.9, help="decoder beta 2"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5000,
        help="test set batch size (default: 5000)",
    )
    parser.add_argument(
        "--load_mse_model_path",
        type=str,
        default=None,
        help="directory from which to preload enc1/dec1+disc1 models to start training at",
    )
    parser.add_argument(
        "--load_base_model_path",
        type=str,
        default=None,
        help="directory from which to preload enc1/dec1+disc1 models to start training at",
    )
    parser.add_argument(
        "--load_reduced_model_path",
        type=str,
        default=None,
        help="(for joint_reduced_reduced) directory from which to preload discriminator models to start training at",
    )
    parser.add_argument(
        "--initialize_base_discriminator",
        type=int,
        default=0,
        help="For refined or reduced models: whether to start from base model disc.",
    )
    parser.add_argument(
        "--initialize_mse_model",
        type=int,
        default=0,
        help="For base model: whether or not to continue training from Lambda=0 model.",
    )
    parser.add_argument(
        "--enc_layer_scale",
        type=float,
        default=1.0,
        help="Scale layer size of encoder by factor",
    )
    parser.add_argument(
        "--enc_2_layer_scale",
        type=float,
        default=1.0,
        help="Scale layer size of encoder 2 by factor",
    )
    parser.add_argument("--reduced_dims", type=str, default="", help="Reduced dims")
    parser.add_argument(
        "--reduced_path",
        type=str,
        help="If joint reduced training, where to save the secondary model",
    )
    parser.add_argument(
        "--refined_path",
        type=str,
        help="If joint refined training, where to save the secondary model",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="periodically show progress of training",
    )
    parser.add_argument(
        "--entropy_intervals",
        type=int,
        default=-1,
        help="periodically calculate entropy of model. -1 only end, -2 for never",
    )
    parser.add_argument(
        "--submode", type=str, default=None, help="generic submode of mode"
    )
    parser.add_argument(
        "-mode", type=str, default="base", help="base, refined or reduced training mode"
    )
    parser.add_argument(
        "-experiment_path", type=str, help="name of the subdirectory to save"
    )
