...
def visualize_with_dims(dims, encoder, decoder, data_loader, ls_sample_dir):
    ...
    for i, dim in enumerate(dims):
        ...
        x_concat = torch.cat(
            [x.view(-1, 1, 28, 28), x_reconst_dim.view(-1, 1, 28, 28)], dim=3
        )
...
def latent_selection_reconst(args, device):
    ...
    decoder1.load_state_dict(
        torch.load(os.path.join(args.load_base_model_path, "decoder1.ckpt"))
    )
    ...
    train_dataloader, _ = load_dataset(
        args.dataset, args.batch_size, args.batch_size, shuffle_train=False
    )
    ...
    criterion = lambda x, x_reconst: selection_criterion(
        x, x_reconst, args, discriminator1
    )
    latent_selected = []
        ...
        for t in range(latent_dim_0):
            for x, _ in train_dataloader:
                ...
                for dim in latent_unselected:
                    ...
                    latent_unselected_minus_dim = list(
                        deepcopy(latent_unselected)
                        - {
                            dim,
                        }
                    )
                    ...
                    z_copy[:, latent_unselected_minus_dim] = latent_sampler(
                        z[:, latent_unselected_minus_dim]
                    )
            ...
            latent_selected.append(
                int(min_error_dim)
            )  # Can't JSON Serialize numpy objects
            latent_unselected -= {
                min_error_dim,
            }
    ...
    visualize_with_dims(
        latent_selected, encoder1, decoder1, train_dataloader, ls_sample_dir
    )
...
def save_info(selection_method, info_dict, args):
    if os.path.isfile(
        os.path.join(args.load_base_model_path, "_latent_selection.json")
    ):
        with open(
            os.path.join(args.load_base_model_path, "_latent_selection.json"), "r"
        ) as f:
            data = json.load(f)
    ...
    with open(
        os.path.join(args.load_base_model_path, "_latent_selection.json"), "w"
    ) as f:
        json.dump(data, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_channel", type=int, default=1, help="hidden dimension of z (default: 8)"
    )
    parser.add_argument(
        "--latent_dim_1", type=int, default=8, help="hidden dimension of z (default: 8)"
    )
    parser.add_argument(
        "--latent_dim_0",
        type=int,
        default=-1,
        help="selection size of latent dimension",
    )
    parser.add_argument(
        "--quantize", type=int, default=1, help="DOES NOTHING RIGHT NOW"
    )
    parser.add_argument(
        "--stochastic",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="coefficient for selection loss",
    )
    parser.add_argument(
        "--Lambda_base", type=float, default=0.0, help="coefficient for selection loss"
    )
    parser.add_argument(
        "--batch_size", type=int, default=5000, help="batch size (default: 5000)"
    )
    parser.add_argument(
        "--enc_layer_scale",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="train",
        help="use train set or test set (DOES NOTHING; always uses train)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        help="Load pretrained base model for selection",
    )
    parser.add_argument(
        "-method",
        type=str,
        help="Method (reconstruction loss or dimensionwise entropy)",
    )
