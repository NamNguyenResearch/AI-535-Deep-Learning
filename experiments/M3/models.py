...
class DecoderMNIST(nn.Module):
    def __init__(self, latent_dim, output_size):
        ...
        if self.output_size == 784:
            ...
            raise ValueError(
                f"No deconvolution defined for output size of {self.output_size}."
            )
...
class EncoderLSUN(nn.Module):
    def __init__(
        ...
        self.main = nn.Sequential(
            [
                nn.Conv2d(
                    self.n_channel, 64, kernel_size=4, stride=2, padding=1, bias=False
                ),
...
class DecoderLSUN(nn.Module):
    def __init__(self, latent_dim, output_size):
        ...
        self.deconvolve = nn.Sequential(
            [
                nn.ConvTranspose2d(
                    128, 512, kernel_size=(4, 4), stride=(1, 1), bias=False
                ),
                BatchNorm2d(
...
class DiscriminatorLSUN(nn.Module):
    def __init__(self, n_channel):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(
                64, 64, kernel_size=(4, 4), stride=(2, 2), padding=(1, 1), bias=False
            ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-input_size", type=int, default=784, help="hidden dimension (default: 128)"
    )
    parser.add_argument(
        "-latent_dim_1", type=int, default=6, help="hidden dimension of z (default: 8)"
    )
    parser.add_argument(
        "-L_1", type=int, default=3, help="number of quantization levels (default: 4"
    )
    parser.add_argument(
        "-n_channel", type=int, default=1, help="input channels (default: 1)"
    )
    parser.add_argument(
        "-quantize", type=bool, default=True, help="do quantization (default: True)"
    )
    parser.add_argument(
        "-stochastic",
        type=bool,
    ...
    parser.add_argument(
        "-enc_layer_scale", type=float, default=1.0, help="layer factor for encoder"
    )
