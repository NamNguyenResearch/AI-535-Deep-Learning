...
def load_dataset(
    dataset, batch_size, test_batch_size, shuffle_train=True, shuffle_test=False
):
    if dataset.lower() == "mnist":
        train_dataloader, test_dataloader, unnormalizer = load_mnist(
            batch_size,
            test_batch_size,
        ...
        train_dataloader, test_dataloader, unnormalizer = load_fashion_mnist(
            batch_size,
            test_batch_size,
            shuffle_train=shuffle_train,
            shuffle_test=shuffle_test,
        ...
        train_dataloader, test_dataloader, unnormalizer = load_svhn(
            batch_size,
            test_batch_size,
            shuffle_train=shuffle_train,
            shuffle_test=shuffle_test,
        ...
        train_dataloader, test_dataloader, unnormalizer = load_lsun(
            batch_size,
            test_batch_size,
            shuffle_train=shuffle_train,
            shuffle_test=shuffle_test,
...
def load_fashion_mnist(
    batch_size, test_batch_size, shuffle_train=True, shuffle_test=False
):
    train_dataloader = torch.utils.data.DataLoader(
...
def uniform_noise_like(
    x, alpha: float, epsilon: float = 0, device=torch.device("cuda")
):
    return (
        torch.FloatTensor(*(x.shape))
        .uniform_(-alpha / 2 + epsilon, +alpha / 2 - epsilon)
        .to(device)
    )
...
def get_secondary_model_dirs(Lambda_dirs, filtering=None):
    ...
    for secondary_root in Lambda_dirs:
        ...
        latent_dim_1, L_1 = (
            int(primary_latent_dir_details[0]),
            int(primary_latent_dir_details[1]),
        )
        try:
        ...
        for latent_dir in latent_dirs:
            ...
            if is_model:
                ...
                ):  # reduced_only with same-dimension latents
                    latent_dir_details = latent_dir.split("-")
                    if len(latent_dir_details) == 2:
                        print(f"Incompatible directory: {latent_dir}")
                    elif len(latent_dir_details) == 3:
                        ...
                        latent_dim_0, L_0 = (
                            int(latent_dir_details[0]),
                            int(latent_dir_details[1]),
                        )
                        if (
                            latent_dim_1 != latent_dim_0 or L_1 != L_0
                        ):  # if not same dimension/rate decoder
                            continue
                ...
                sub_Lambda_dirs_full = [
                    os.path.join(latent_dir_full, sub_Lambda_dir)
...
def get_model_dirs(parent_dir, filtering=None):
    model_dirs, experiment_dirs = get_base_model_dirs(
        parent_dir=parent_dir, return_model_dirs=True
    )
...
def assert_args_match(args1, args2, matching_args):
    ...
    for arg in matching_args:
        assert arg in args1 and arg in args2, (
            f"{arg} in args1: {arg in args1}, {arg} in args2: {arg in args2}"
        )
        assert args1[arg] == args2[arg], (
            f"Args don't match: {args1[arg]} != {args2[arg]}, {arg}"
        )
...
def calculate_rate(
    mode, latent_dim_1, L_1, latent_dim_2=-1, L_2=-1, latent_dim_0=-1, L_0=-1
):
    n_bits = None
    if mode == "base":
        ...
        if L_1 > 0 and L_2 > 0:
            n_bits = latent_dim_1 * math.log2(L_1) + latent_dim_2 * math.log2(L_2)
    elif mode == "reduced":
        if L_1 > 0 and L_0 > 0:
