    ...
    for setting in settings:
        ...
        if mode == "base":
            experiment_path = os.path.join(
                experiment_path_pre, experiment_path_base, base_Lambda_dir
            )
            if setting["Lambda_base"] > 0:
                # experiment_path = os.path.join(experiment_path_base, '_MSE')
                # setting['load_mse_model_dir'] = None
                setting["load_mse_model_path"] = os.path.join(
                    experiment_path_pre, experiment_path_base, "Lambda=0.00000"
                )
        elif mode == "refined":
            ...
            setting["load_base_model_path"] = os.path.join(
                experiment_path_pre, experiment_path_base, base_Lambda_dir
            )
        elif mode == "reduced":
            ...
            with open(
                os.path.join(setting["load_base_model_path"], "_latent_selection.json"),
                "r",
            ) as f:
                data = json.load(f)
                # selection_method = setting['selection_method']
                all_selected_dims = data[selection_method][
                    "latent_selected"
                ]  # List of all selected dims
                reduced_dims = all_selected_dims[
                    : setting["latent_dim_0"]
                ]  # Choose how many
            reduced_dims = ",".join([str(dim) for dim in reduced_dims])
            setting["reduced_dims"] = reduced_dims
        elif mode == "joint_reduced":
            assert (
                setting["Lambda_reduced"] == setting["Lambda_base"]
            )  # otherwise will overwrite base file
            ...
            assert (
                setting["Lambda_refined"] == setting["Lambda_base"]
            )  # otherwise will overwrite base file
            ...
            experiment_path_base += f"-{setting['latent_dim_0']}-{setting['L_0']}"  # z_dim_0-L_0-z_dim_1-L_1
            reduced_Lambda_dir = "Lambda={:.5f}".format(setting["Lambda_reduced"])
            reduced_reduced_Lambda_dir = "Lambda={:.5f}".format(
                setting["Lambda_reduced_reduced"]
            )
            setting["load_base_model_path"] = os.path.join(
                experiment_path_pre, experiment_path_base, base_Lambda_dir
            )
            if submode == "joint_reduced_reduced_1":
                assert (setting["latent_dim_M1"], setting["L_M1"]) == (
                    setting["latent_dim_1"],
                    setting["L_1"],
                )  # only use for reducing to same dim for now
                experiment_path = os.path.join(
                    setting["load_base_model_path"],
                ...
                setting["load_reduced_model_path"] = os.path.join(
                    setting["load_base_model_path"],
                    f"{setting['latent_dim_0']}-{setting['L_0']}-[joint_reduced]",
                    reduced_Lambda_dir,
                )
                experiment_path = os.path.join(
                    setting["load_reduced_model_path"],
...
def select_dims(mode, settings, method, experiment_number):
    ...
    for setting in settings:
        ...
        commands.append(
            "python latent_selection.py "
            + " ".join(f"--{key} {value}" for key, value in setting.items())
            + f" -load_base_model_path {load_model_dir}"
            + f" -method {method}"
        )
...
def evaluate_quantized_entropy(mode, settings, method, experiment_number):
    ...
    for setting in settings:
        experiment_path_base = f"{setting['latent_dim_1']}-{setting['L_1']}"
        base_Lambda_dir = "Lambda={:.5f}".format(setting["Lambda_base"])
        load_base_model_path = os.path.join(
            experiment_path_pre, experiment_path_base, base_Lambda_dir
        )
    ...
    print(
        f"Done evaluate_quantized_entropy({mode}, {str(settings)[:25]}... , {experiment_number})"
    )
...
def evaluate_latent_variance(mode, settings, experiment_number):
    ...
    run(experiment_path_pre, settings, commands)
    print(
        f"Done evaluate_latent_variance({mode}, {str(settings)[:25]}... , {experiment_number})"
    )
